...
from maddpg_gnn.maddpg_gnn_agent import maddpg_gnn_agent
from maddpg_gnn.maddpg_gnn_replay_buffer import maddpg_gnn_replay_buffer
...
class maddpg_gnn:
    ...
    def __init__(self, mode, scenario_name, training_name, discount_rate, lr_actor, lr_critic, num_agents, num_opp, actor_dropout_p, critic_dropout_p, state_fc_input_dims, state_fc_output_dims, 
                 ...
                 c_actions_fc_input_dims, c_actions_fc_output_dims, concat_fc_output_dims, tau, mem_size, batch_size, update_target, grad_clipping, grad_norm_clip, is_adversary):
            
        """ class constructor for attributes of the maddpg class (for multiple agents) """
        
        # list to store maddpg agents
        self.maddpg_gnn_agents_list = []
        ...
        if is_adversary == False:
            ...
            for i in range(num_agents):
                ...
                self.maddpg_gnn_agents_list.append(maddpg_gnn_agent(mode = mode, scenario_name = scenario_name, training_name = training_name, discount_rate = discount_rate, lr_actor = lr_actor, 
                                                                    ...
                                                                    gnn_input_dims = gnn_input_dims[i], gnn_output_dims = gnn_output_dims, gmt_hidden_dims = gmt_hidden_dims, 
                                                                    ...
                                                                    c_actions_fc_output_dims = c_actions_fc_output_dims, concat_fc_output_dims = concat_fc_output_dims, tau = tau))
                
                # update actor model_names attributes for checkpoints
                self.maddpg_gnn_agents_list[i].maddpg_gnn_actor.model_name = "maddpg_gnn_actor"
        
                # update actor checkpoints_path attributes
                self.maddpg_gnn_agents_list[i].maddpg_gnn_actor.checkpoint_path = os.path.join(self.maddpg_gnn_agents_list[i].maddpg_gnn_actor.checkpoint_dir, 
                                                                                               self.maddpg_gnn_agents_list[i].maddpg_gnn_actor.model_name + "_" + str(i) + ".pt")
                
                # update target actor model_names attributes for checkpoints
                self.maddpg_gnn_agents_list[i].maddpg_gnn_target_actor.model_name = "maddpg_gnn_target_actor"
        
                # update target actor checkpoints_path attributes
                self.maddpg_gnn_agents_list[i].maddpg_gnn_target_actor.checkpoint_path = os.path.join(self.maddpg_gnn_agents_list[i].maddpg_gnn_target_actor.checkpoint_dir, 
                                                                                                      self.maddpg_gnn_agents_list[i].maddpg_gnn_target_actor.model_name + "_" + str(i) + ".pt")
                
                # update critic model_names attributes for checkpoints
                self.maddpg_gnn_agents_list[i].maddpg_gnn_critic.model_name = "maddpg_gnn_critic"
        
                # update critic checkpoints_path attributes
                self.maddpg_gnn_agents_list[i].maddpg_gnn_critic.checkpoint_path = os.path.join(self.maddpg_gnn_agents_list[i].maddpg_gnn_critic.checkpoint_dir, 
                                                                                                self.maddpg_gnn_agents_list[i].maddpg_gnn_critic.model_name + "_" + str(i) + ".pt")
                
                # update target critic model_names attributes for checkpoints
                self.maddpg_gnn_agents_list[i].maddpg_gnn_target_critic.model_name = "maddpg_gnn_target_critic"
        
                # update target critic checkpoints_path attributes
                self.maddpg_gnn_agents_list[i].maddpg_gnn_target_critic.checkpoint_path = os.path.join(self.maddpg_gnn_agents_list[i].maddpg_gnn_target_critic.checkpoint_dir, 
                                                                                                       self.maddpg_gnn_agents_list[i].maddpg_gnn_target_critic.model_name + "_" + str(i) + ".pt")

            # if mode is not test
            if mode == 'train':
                
                # create replay buffer
                self.replay_buffer = maddpg_gnn_replay_buffer(mem_size = mem_size, num_agents = num_agents, u_actions_dims = u_action_dims, c_actions_dims = c_action_dims, 
    ...
    def select_actions(self, mode, env_agents, actor_state_list):
        ...
        # iterate over num_agents
        for agent_index, agent in enumerate(self.maddpg_gnn_agents_list):
    ...
    def apply_gradients_maddpg_gnn(self, num_of_agents):
        ...
        device = self.maddpg_gnn_agents_list[0].maddpg_gnn_actor.device
        ...
        for agent_index, agent in enumerate(self.maddpg_gnn_agents_list):
            ...
            agent.maddpg_gnn_actor.eval()
            agent.maddpg_gnn_critic.eval()
            agent.maddpg_gnn_target_actor.eval()
            agent.maddpg_gnn_target_critic.eval()
            ...
            curr_target_actor_u_actions_prime, curr_target_actor_c_actions_prime = agent.maddpg_gnn_target_actor.forward(actor_state_prime)
            ...
            curr_actor_u_actions, curr_actor_c_actions = agent.maddpg_gnn_actor.forward(actor_state)
        ...
        for agent_index, agent in enumerate(self.maddpg_gnn_agents_list):
            ...
            target_critic_q_value_prime = agent.maddpg_gnn_target_critic.forward(critic_state_prime, batch, curr_target_actor_u_actions_prime_cat, curr_target_actor_c_actions_prime_cat).flatten()
            
            # mask terminal target q values with 0
            target_critic_q_value_prime[terminal[:, 0]] = 0.0
            
            # obtain critic q value
            critic_q_value = agent.maddpg_gnn_critic.forward(critic_state, batch, past_actor_u_actions_cat, past_actor_c_actions_cat).flatten()
            ...
            agent.maddpg_gnn_critic.train()
             
            # reset gradients for critic model to zero
            agent.maddpg_gnn_critic.optimizer.zero_grad()
            ...
            if self.grad_clipping == True:
                ...
                critic_grad_norm = T.nn.utils.clip_grad_norm_(agent.maddpg_gnn_critic.parameters(), max_norm = self.grad_norm_clip, norm_type = 2, error_if_nonfinite = True)

            # apply gradients to critic model
            agent.maddpg_gnn_critic.optimizer.step()
            
            # set critic to eval mode to calculate actor loss
            agent.maddpg_gnn_critic.eval()
            
            # set actor model to train mode 
            agent.maddpg_gnn_actor.train()
            ...
            actor_loss = agent.maddpg_gnn_critic.forward(critic_state, batch, curr_actor_u_actions_cat, curr_actor_c_actions_cat).flatten()
           
            # reduce mean across batch_size
            actor_loss = -T.mean(actor_loss)
            
            # reset gradients for actor model to zero
            agent.maddpg_gnn_actor.optimizer.zero_grad()

            # actor model back propagation
            actor_loss.backward(retain_graph = True, inputs = list(agent.maddpg_gnn_actor.parameters()))
            
            # check if gradient clipping is needed
            if self.grad_clipping == True:
            
                # gradient norm clipping for critic model
                actor_grad_norm = T.nn.utils.clip_grad_norm_(agent.maddpg_gnn_actor.parameters(), max_norm = self.grad_norm_clip, norm_type = 2, error_if_nonfinite = True)

            # apply gradients to actor model
            agent.maddpg_gnn_actor.optimizer.step()
            ...
            if self.update_target == None:
                ...
                # iterate over agents
                for agent in self.maddpg_gnn_agents_list:

                    # update target models
                    agent.update_maddpg_gnn_target_models()    
