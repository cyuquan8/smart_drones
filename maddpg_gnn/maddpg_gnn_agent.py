# ==========================================================================================================================================================
# maddpg gnn agent class 
# purpose: creates and updates neural network 
# ==========================================================================================================================================================

import torch as T
from nn import maddpg_mlp_actor_model, maddpg_gnn_critic_model

class maddpg_gnn_agent:
    ...
    def __init__(self, mode, scenario_name, training_name, discount_rate, lr_actor, lr_critic, num_agents, num_opp, actor_dropout_p, critic_dropout_p, state_fc_input_dims, state_fc_output_dims, 
                 ...
                 c_actions_fc_input_dims, c_actions_fc_output_dims, concat_fc_output_dims, tau):
        ...
        self.maddpg_gnn_actor = maddpg_mlp_actor_model(model = "maddpg_gnn_actor", model_name = None, mode = mode, scenario_name = scenario_name, training_name = training_name, 
                                                       learning_rate = self.lr_actor, num_agents = num_agents, num_opp = num_opp, dropout_p = actor_dropout_p, fc_input_dims = state_fc_input_dims, 
        ...
        self.maddpg_gnn_target_actor = maddpg_mlp_actor_model(model = "maddpg_gnn_actor", model_name = None, mode = mode, scenario_name = scenario_name, training_name = training_name, 
                                                              learning_rate = self.lr_actor, num_agents = num_agents, num_opp = num_opp, dropout_p = actor_dropout_p, 
                                                              fc_input_dims = state_fc_input_dims, fc_output_dims = state_fc_output_dims, tanh_actions_dims = u_action_dims, 
                                                              sig_actions_dims = c_action_dims)

        # intialise critic model
        self.maddpg_gnn_critic = maddpg_gnn_critic_model(model = "maddpg_gnn_critic", model_name = None, mode = mode, scenario_name = scenario_name, training_name = training_name, 
        ...
        self.maddpg_gnn_target_critic = maddpg_gnn_critic_model(model = "maddpg_gnn_critic", model_name = None, mode = mode, scenario_name = scenario_name, training_name = training_name, 
                                                                learning_rate = self.lr_critic, num_agents = num_agents, num_opp = num_opp, num_heads = num_heads, bool_concat = bool_concat, 
                                                                dropout_p = critic_dropout_p, gnn_input_dims = gnn_input_dims, gnn_output_dims = gnn_output_dims, gmt_hidden_dims = gmt_hidden_dims, 
                                                                ...
                                                                c_actions_fc_output_dims = c_actions_fc_output_dims, concat_fc_output_dims = concat_fc_output_dims)

        # hard update target models' weights to online network to match initialised weights
        self.update_maddpg_gnn_target_models(tau = 1)
        
    def update_maddpg_gnn_target_models(self, tau = None): 
        ...
        for target_actor_parameters, actor_parameters in zip(self.maddpg_gnn_target_actor.parameters(), self.maddpg_gnn_actor.parameters()):
        ...
        for target_critic_parameters, critic_parameters in zip(self.maddpg_gnn_target_critic.parameters(), self.maddpg_gnn_critic.parameters()):
    ...
    def select_action(self, mode, agent, state):
        ...
        self.maddpg_gnn_actor.eval()
        
        # turn actor local state observations to tensor in actor device
        actor_input = T.tensor(state, dtype = T.float).to(self.maddpg_gnn_actor.device)
        ...
        u_action, c_action = self.maddpg_gnn_actor.forward(actor_input)
    ...
    def save_models(self):
        ...
        T.save(self.maddpg_gnn_actor.state_dict(), self.maddpg_gnn_actor.checkpoint_path)
        T.save(self.maddpg_gnn_target_actor.state_dict(), self.maddpg_gnn_target_actor.checkpoint_path)
        T.save(self.maddpg_gnn_critic.state_dict(), self.maddpg_gnn_critic.checkpoint_path)
        T.save(self.maddpg_gnn_target_critic.state_dict(), self.maddpg_gnn_target_critic.checkpoint_path)
    ...
    def load_models(self):
        ...
        self.maddpg_gnn_actor.load_state_dict(T.load(self.maddpg_gnn_actor.checkpoint_path, map_location = T.device('cuda:0' if T.cuda.is_available() else 'cpu')))
        self.maddpg_gnn_target_actor.load_state_dict(T.load(self.maddpg_gnn_target_actor.checkpoint_path, map_location = T.device('cuda:0' if T.cuda.is_available() else 'cpu')))
        self.maddpg_gnn_critic.load_state_dict(T.load(self.maddpg_gnn_critic.checkpoint_path, map_location = T.device('cuda:0' if T.cuda.is_available() else 'cpu')))
        self.maddpg_gnn_target_critic.load_state_dict(T.load(self.maddpg_gnn_target_critic.checkpoint_path, map_location = T.device('cuda:0' if T.cuda.is_available() else 'cpu')))