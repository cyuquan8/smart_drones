...
from maddpgv2.maddpgv2_agent import maddpgv2_agent
from maddpgv2.maddpgv2_replay_buffer import maddpgv2_replay_buffer
...
class maddpgv2:
    ...
    def __init__(self, mode, training_name, discount_rate, lr_actor, lr_critic, num_agents, num_opp, actor_dropout_p, critic_dropout_p, state_fc_input_dims, state_fc_output_dims, u_action_dims, 
                 ...
                 c_actions_fc_output_dims, concat_fc_output_dims, goal_fc_input_dims, goal_fc_output_dims, tau, mem_size, batch_size, update_target, grad_clipping, grad_norm_clip, num_of_add_goals, 
                 goal_strategy, is_adversary, ep_time_limit, r_rad, big_rew_cnst, rew_multiplier_cnst, pos_dims, exit_screen_terminate):
            
        """ class constructor for attributes of the maddpg class (for multiple agents) """
        
        # list to store maddpg agents
        self.maddpgv2_agents_list = []
        ...
        if is_adversary == False:
            ...
            for i in range(num_agents):
                ...
                self.maddpgv2_agents_list.append(maddpgv2_agent(mode = mode, training_name = training_name, discount_rate = discount_rate, lr_actor = lr_actor, lr_critic = lr_critic, 
                                                                ...
                                                                goal_fc_input_dims = goal_fc_input_dims, goal_fc_output_dims = goal_fc_output_dims, concat_fc_output_dims = concat_fc_output_dims, 
                                                                tau = tau, is_adversary = is_adversary))
                
                # update actor model_names attributes for checkpoints
                self.maddpgv2_agents_list[i].maddpgv2_actor.model_name = "maddpgv2_actor"
        
                # update actor checkpoints_path attributes
                self.maddpgv2_agents_list[i].maddpgv2_actor.checkpoint_path = os.path.join(self.maddpgv2_agents_list[i].maddpgv2_actor.checkpoint_dir, 
                                                                                           self.maddpgv2_agents_list[i].maddpgv2_actor.model_name + "_" + str(i) + ".pt")
                
                # update target actor model_names attributes for checkpoints
                self.maddpgv2_agents_list[i].maddpgv2_target_actor.model_name = "maddpgv2_target_actor"
        
                # update target actor checkpoints_path attributes
                self.maddpgv2_agents_list[i].maddpgv2_target_actor.checkpoint_path = os.path.join(self.maddpgv2_agents_list[i].maddpgv2_target_actor.checkpoint_dir, 
                                                                                                  self.maddpgv2_agents_list[i].maddpgv2_target_actor.model_name + "_" + str(i) + ".pt")
                
                # update critic model_names attributes for checkpoints
                self.maddpgv2_agents_list[i].maddpgv2_critic.model_name = "maddpgv2_critic"
        
                # update critic checkpoints_path attributes
                self.maddpgv2_agents_list[i].maddpgv2_critic.checkpoint_path = os.path.join(self.maddpgv2_agents_list[i].maddpgv2_critic.checkpoint_dir, 
                                                                                            self.maddpgv2_agents_list[i].maddpgv2_critic.model_name + "_" + str(i) + ".pt")
                
                # update target critic model_names attributes for checkpoints
                self.maddpgv2_agents_list[i].maddpgv2_target_critic.model_name = "maddpgv2_target_critic"
        
                # update target critic checkpoints_path attributes
                self.maddpgv2_agents_list[i].maddpgv2_target_critic.checkpoint_path = os.path.join(self.maddpgv2_agents_list[i].maddpgv2_target_critic.checkpoint_dir, 
                                                                                                   self.maddpgv2_agents_list[i].maddpgv2_target_critic.model_name + "_" + str(i) + ".pt")

                # if mode is not test
                if mode == 'train':
                    
                    # create replay buffer
                    self.replay_buffer = maddpgv2_replay_buffer(mem_size = mem_size, num_agents = num_agents, u_actions_dims = u_action_dims, c_actions_dims = c_action_dims, 
                                                                actor_input_dims = state_fc_input_dims, goal_dims = goal_fc_input_dims, num_of_add_goals = num_of_add_goals, 
                                                                goal_strategy = goal_strategy, is_adversary = is_adversary, ep_time_limit = ep_time_limit, r_rad = r_rad, big_rew_cnst = big_rew_cnst, 
                                                                rew_multiplier_cnst = rew_multiplier_cnst, pos_dims = pos_dims, exit_screen_terminate = exit_screen_terminate)
        ...
        # check if agent
            ...
            for i in range(num_opp):
                ...
                if mode == 'train':
                    ...
                    # create replay buffer
                    self.replay_buffer = maddpgv2_replay_buffer(mem_size = mem_size, num_agents = num_opp, u_actions_dims = u_action_dims, c_actions_dims = c_action_dims, 
                                                                actor_input_dims = state_fc_input_dims, goal_dims = goal_fc_input_dims, num_of_add_goals = num_of_add_goals, 
                                                                goal_strategy = goal_strategy, is_adversary = is_adversary, ep_time_limit = ep_time_limit, r_rad = r_rad, big_rew_cnst = big_rew_cnst, 
                                                                rew_multiplier_cnst = rew_multiplier_cnst, pos_dims = pos_dims, exit_screen_terminate = exit_screen_terminate)
    ...
    def select_actions(self, mode, env_agents, actor_state_list):
        ...
        # iterate over num_agents
        for agent_index, agent in enumerate(self.maddpgv2_agents_list):
    ...
    def apply_gradients_maddpgv2(self, num_of_agents):
        ...
        device = self.maddpgv2_agents_list[0].maddpgv2_actor.device
        ...
        for agent_index, agent in enumerate(self.maddpgv2_agents_list):
            ...
            agent.maddpgv2_actor.eval()
            agent.maddpgv2_critic.eval()
            agent.maddpgv2_target_actor.eval()
            agent.maddpgv2_target_critic.eval()
            ...
            curr_target_actor_u_actions_prime, curr_target_actor_c_actions_prime = agent.maddpgv2_target_actor.forward(actor_state_prime)
            ...
            curr_actor_u_actions, curr_actor_c_actions = agent.maddpgv2_actor.forward(actor_state)
        ...
        for agent_index, agent in enumerate(self.maddpgv2_agents_list):
            ...
            target_critic_q_value_prime = agent.maddpgv2_target_critic.forward(critic_state_prime, batch, curr_target_actor_u_actions_prime_cat, curr_target_actor_c_actions_prime_cat, 
            ...
            critic_q_value = agent.maddpgv2_critic.forward(critic_state, batch, past_actor_u_actions_cat, past_actor_c_actions_cat, critic_goals).flatten()
            ...
            agent.maddpgv2_critic.train()
             
            # reset gradients for critic model to zero
            agent.maddpgv2_critic.optimizer.zero_grad()
            ...
            if self.grad_clipping == True:
                ...
                critic_grad_norm = T.nn.utils.clip_grad_norm_(agent.maddpgv2_critic.parameters(), max_norm = self.grad_norm_clip, norm_type = 2, error_if_nonfinite = True)

            # apply gradients to critic model
            agent.maddpgv2_critic.optimizer.step()
            
            # set critic to eval mode to calculate actor loss
            agent.maddpgv2_critic.eval()
            
            # set actor model to train mode 
            agent.maddpgv2_actor.train()
            ...
            actor_loss = agent.maddpgv2_critic.forward(critic_state, batch, curr_actor_u_actions_cat, curr_actor_c_actions_cat, critic_goals).flatten()
           
            # reduce mean across batch_size
            actor_loss = -T.mean(actor_loss)
            
            # reset gradients for actor model to zero
            agent.maddpgv2_actor.optimizer.zero_grad()

            # actor model back propagation
            actor_loss.backward(retain_graph = True, inputs = list(agent.maddpgv2_actor.parameters()))
            
            # check if gradient clipping is needed
            if self.grad_clipping == True:
            
                # gradient norm clipping for critic model
                actor_grad_norm = T.nn.utils.clip_grad_norm_(agent.maddpgv2_actor.parameters(), max_norm = self.grad_norm_clip, norm_type = 2, error_if_nonfinite = True)

            # apply gradients to actor model
            agent.maddpgv2_actor.optimizer.step()
            ...
            if self.update_target == None:
                ...
                for agent in self.maddpgv2_agents_list:
                    ...
                    agent.update_maddpgv2_target_models()    
