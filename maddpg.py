...
from maddpg_replay_buffer import maddpg_replay_buffer
...
class maddpg:
    ...
    def apply_gradients_maddpg(self):
        ...
        batch = T.tensor([i for i in range(self.batch_size) for j in range(self.num_agents + self.num_opp)], dtype = T.long).to(device)
        ...
        # list to store actor and critic loss for logging
        actor_loss_list = []
        critic_loss_list = []
        ...
        for agent_index, agent in enumerate(self.maddpg_agents_list):
            ...
            if self.grad_clipping == True:
                ...
                T.nn.utils.clip_grad_norm_(agent.maddpg_critic.parameters(), max_norm = self.grad_norm_clip, norm_type = 2)
            ...
            if self.grad_clipping == True:
                ...
                # gradient norm clipping for critic model
                T.nn.utils.clip_grad_norm_(agent.maddpg_actor.parameters(), max_norm = self.grad_norm_clip, norm_type = 2)
            ...
            # store actor and critic loss in list
