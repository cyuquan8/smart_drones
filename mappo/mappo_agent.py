...
import torch as T
from nn import mappo_actor_model, mappo_critic_model
...
class mappo_agent:
    ...
    def __init__(self, mode, training_name, lr_actor, lr_critic, num_agents, num_opp, u_range, u_noise, c_noise, is_adversary, actor_dropout_p, critic_dropout_p, state_fc_input_dims, 
        ...
        self.mappo_actor = mappo_actor_model(model = "mappo_actor", model_name = None, mode = mode, training_name = training_name, learning_rate = self.lr_actor, num_agents = num_agents, 
                                             num_opp = num_opp, u_range = u_range, u_noise = u_noise, c_noise = c_noise, is_adversary = is_adversary, dropout_p = actor_dropout_p, 
                                             fc_input_dims = state_fc_input_dims, fc_output_dims = state_fc_output_dims, tanh_actions_dims = u_action_dims, sig_actions_dims = c_action_dims)

        # intialise critic model
        self.mappo_critic = mappo_critic_model(model = "mappo_critic", model_name = None, mode = mode, training_name = training_name, learning_rate = self.lr_critic, num_agents = num_agents, 
                                               num_opp = num_opp, num_heads = num_heads, bool_concat = bool_concat, dropout_p = critic_dropout_p, gnn_input_dims = gnn_input_dims, 
                                               gnn_output_dims = gnn_output_dims, gmt_hidden_dims = gmt_hidden_dims, gmt_output_dims = gmt_output_dims, fc_output_dims = fc_output_dims)
    ...
    def select_action(self, mode, agent, state):
        ...
        self.mappo_actor.eval()
        
        # turn actor local state observations to tensor in actor device
        actor_input = T.tensor(state, dtype = T.float).to(self.mappo_actor.device)
        ...
        u_action_norm_dist, c_action_norm_dist = self.mappo_actor.forward(actor_input)

        # set actor model to training mode (for batch norm and dropout)
        self.mappo_actor.train()
    ...
    def save_models(self):
        ...
        T.save(self.mappo_actor.state_dict(), self.mappo_actor.checkpoint_path)
        T.save(self.mappo_critic.state_dict(), self.mappo_critic.checkpoint_path)
    ...
    def load_models(self):
        ...
        self.mappo_actor.load_state_dict(T.load(self.mappo_actor.checkpoint_path, map_location = T.device('cuda:0' if T.cuda.is_available() else 'cpu')))
        self.mappo_critic.load_state_dict(T.load(self.mappo_critic.checkpoint_path, map_location = T.device('cuda:0' if T.cuda.is_available() else 'cpu')))